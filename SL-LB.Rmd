...
```{r}
mleaf <- lmer(T_Longest_leaf_lg ~ Treatment +(1 |Pop), data = smart)
anova(mleaf, type=3) #no interaction
#plot(mleaf)
```

Stem width
```{r}
mstem <-  lmer(T_Stem_width_mum ~ Treatment +(1 |Pop), data = smart)
anova(mstem, type=3) 
#plot(mstem)
```
...
*Phenology*
First day of pollen production (male function)
```{r}
mpol <-  lmer(T_pol_day ~ Treatment +(1 |Pop), data = smart)
anova(mpol, type=3) 
#plot(mpol)
```

Length of male function flowering
```{r}
mmlen <-  lmer(T_male_length_sq ~ Treatment +(1 |Pop), data = smart)
anova(mmlen, type=3) 
#plot(mmlen)
```

First day of seed structures/styles (female function)
```{r}
mfem <-  lmer(T_fem_day ~ Treatment +(1 |Pop), data = smart)
anova(mfem, type=3) 
#plot(mfem)
```

Length of female function flowering
```{r}
mfeml <-  lmer(T_fem_length_sq ~ Treatment +(1 |Pop), data = smart)
anova(mfeml, type=3) 
#plot(mfeml)
```
...
Start of flowering
```{r}
mflo <-  lmer(T_fl_start ~ Treatment +(1 |Pop), data = smart)
anova(mflo, type=3) 
#plot(mflo)
```
...
Length of  flowering
```{r}
mflol <-  lmer(T_flower_length_lg ~ Treatment +(1 |Pop), data = smart)
anova(mflol, type=3) 
#plot(mflol)
```

First signs of scenescence
```{r}
mdie <-  lmer(T_die_day_lg ~ Treatment +(1 |Pop), data = smart)
anova(mdie, type=3) 
#plot(mdie)
```
...
Day on which the maximum slope is achieved
```{r}
mmaxgroday <-  lmer(T_MaxSlope_cm_Day ~ Treatment +(1 |Pop), data = smart)
anova(mmaxgroday, type=3) 
#plot(mmaxgroday)
```

Length of linear growth
```{r}
mlingro <-  lmer(T_LinearGrowth_Days_sq ~ Treatment +(1 |Pop), data = smart)
anova(mlingro, type=3) 
#plot(mlingro)
```

Inflex point
```{r}
minfx <-  lmer(T_InflxPt_Day_sq ~ Treatment +(1 |Pop), data = smart)
anova(minfx, type=3) 
#plot(minfx)
```
...
*Germination measures*

Weight of mom seeds
```{r}
mmom <-  lmer(T_momseed_mg ~ Treatment +(1 |Pop), data = smart)
anova(mmom, type=3) 
#plot(mmom)
```
...
Germination
```{r}
mgerm <-  lmer(T_germ ~ Treatment +(1 |Pop), data = smart)
anova(mgerm, type=3) 
#plot(mgerm)
```
...
Specific leaf area
```{r}
msla <-  lmer(T_Sla ~ Treatment +(1 |Pop), data = smart)
anova(msla, type=3) 
```
